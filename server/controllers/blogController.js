...
// ❌ REMOVED: This logic crashes Vercel/serverless environments because 
// they have read-only file systems and cannot create folders outside of /tmp.
/*
const uploadDir = 'uploads';
if (!fs.existsSync(uploadDir)) {
    fs.mkdirSync(uploadDir, { recursive: true });
...
export const addBlog = async (req, res) => {
    try {
        ...
        // Validate uploaded file
        // IMPORTANT: Your Multer configuration must be set to save files in the /tmp directory
        // for this to work correctly on Vercel.
        if (!imageFile || !imageFile.path) {
            return res.status(400).json({
                success: false,
                message: "No image file uploaded or file field name is incorrect"
        ...
        // Read file buffer
        // Using fs.promises.readFile is correct for reading the temporary file
        const fileBuffer = await fs.promises.readFile(imageFile.path);
        ...
        // Delete local file after upload
        // This is necessary to clear the /tmp directory and avoid filling up disk space.
        fs.unlink(imageFile.path, (err) => {
            if (err) console.error('Failed to delete local file:', err);
        });
...
export const generateContent = async (req, res) => {
    try {
        ...
        // ✅ The original commented-out code had 'success: false' here, 
        // which has been corrected to 'success: true' in your current block.
